...
export default function PopularFormats() {
  ...
  // Auto-slide logic (Har 3 second me slide change hoga)
  useEffect(() => {
  ...
  const prevSlide = () => {
    setCurrentIndex((prev) => (prev === 0 ? formats.length - 1 : prev - 1));
  };

  return (
    <section className="bg-white py-12 md:py-20 px-4 md:px-8 border-t border-[#e8e0d0]">
      <div className="max-w-[1200px] w-full mx-auto">
        <h2 className="font-cormorant text-xl md:text-3xl tracking-[0.15em] uppercase text-[#1a1a1a] mb-8 md:mb-12 text-center">
          Popular Formats
        </h2>

        {/* --- DESKTOP VIEW (4 Columns Grid - Jaise pehle tha) --- */}
        <div className="hidden md:grid md:grid-cols-4 md:gap-12 md:items-end">
          {formats.map((f, i) => (
            <div key={i} className="flex flex-col items-center gap-4 group cursor-pointer">
              <div className="w-full aspect-[3/4] bg-[#f5f5f5] overflow-hidden rounded-md transition-shadow">
                <img 
                  src={f.image} 
                  alt={f.name} 
        ...
        {/* --- MOBILE VIEW (Auto Slider with Arrows, No Dots) --- */}
        <div className="block md:hidden relative w-full max-w-[280px] mx-auto">
          ...
          <div className="overflow-hidden w-full pb-2">
            ...
            >
              {formats.map((f, i) => (
                <div key={i} className="min-w-full flex flex-col items-center gap-4 px-2">
                  ...
                  <div className="text-center px-1">
                    <p className="font-inter text-[12px] tracking-wide text-[#1a1a1a] uppercase font-medium leading-tight">
                      {f.name}
                    </p>
                    <p className="font-inter text-[12px] text-[#666] mt-1">
                      {f.size}
          ...
          >
            <svg width="18" height="18" fill="none" stroke="#1a1a1a" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
